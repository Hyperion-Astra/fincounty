...
export default function AdjustBalance() {
  ...
  async function handleAdjust(e) {
    ...
    if (Number.isNaN(amt) || amt === 0) return setMsg({ type: "error", text: "Enter a valid amount" });

    setLoading(true);
    try {
      await runTransaction(db, async (t) => {
        const q = query(collection(db, "accounts"), where("uid", "==", uid), where("accountType", "==", accountType));
  ...
  return (
    <div className="adjust-page">
      ...
      <form onSubmit={handleAdjust} className="adjust-form">
        ...
        <input value={uid} onChange={(e) => setUid(e.target.value)} disabled={loading} />
        ...
        <button type="submit" disabled={loading}>{loading ? "Updating..." : "Adjust Balance"}</button>
      </form>
    </div>
  );
}
